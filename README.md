...
```python
问题：这个表格看不懂，比如第2行第1列，对应的数字不是1吗，怎么就牵扯到行标记是1，列标记是0？
...
| i（行） | j（列） | matrix[i][0] 行标记 | matrix[0][j] 列标记 | 结论 |
| ---- | ---- | ---------------- | ---------------- | -- |
| 1    | 1    | matrix[1][0]=0   | matrix[0][1]=0   | 清零 |
| 1    | 2    | matrix[1][0]=0   | matrix[0][2]=1   | 清零 |
| 2    | 1    | matrix[2][0]=1【含义：第 2 行，没有被标记为“整行清零”】   | matrix[0][1]=0【含义：第 1 列，被标记为“整列清零”】   | 清零 |
| 2    | 2    | matrix[2][0]=1   | matrix[0][2]=1   | 保留 |
